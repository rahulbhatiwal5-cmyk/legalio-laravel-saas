...
class GenerateDocumentJob implements ShouldQueue
{
     ...
     */
    public function __construct($documentId, $documentName, $additionalInfo, $fileInput, $isVerified, $documentGeneratorId)
    {
    ...
    public function handle(): void
    {
        ...
        try{
            ...
            $documentGeneratorId = $this->documentGeneratorId;
            ...
            if($aiModel === 'Gemini 2.0' || $aiModel === 'Gemini 2.5 pro'){
                ...
                saveLog("Decoded Gemini:", "GenerateDocumentJob", $decoded);
                ...
                saveLog("chatgpt:", "GenerateDocumentJob", $finalPrompt);
                ...
                if(!$jsonPart){
                    saveLog("Error:", "GenerateDocumentJob", "No valid JSON found in AI output");
                }

                $jsonPart = preg_replace('/,\s*([}\]])/', '$1', $jsonPart);
                $jsonPart = trim($jsonPart);
                $decoded = json_decode($jsonPart, true);
                saveLog("Decoded chatgpt:", "GenerateDocumentJob", $decoded);

            }else{
                saveLog("Error:", "GenerateDocumentJob", "Unsupported AI model: " . $aiModel);
                return;
            }
            
            saveLog("Decoded:", "GenerateDocumentJob", $decoded);
            ...
            foreach($decoded as $sectionBlocks){
                ...
                // saveLog("Questionnaire:", "GenerateDocumentJob", $questionData);
                $rightContentData = $sectionBlocks['Contract_Text'] ?? [];
                // saveLog("Section Blocks:", "GenerateDocumentJob", $rightContentData);
            ...
            saveLog("document generator save:", "GenerateDocumentJob", 'Document generation completed');
    
        }catch(Exception $e){
            saveLog("Error Exception:2323", "GenerateDocumentJob", $e->getMessage());
        }
    }
}
