...
<header class="inner-header fun-header category-searchbar">
	<!-- profile at tab -->
	...
	<div class="main_header">
		<div class="container-fluid">
			<div class="srch-hdr iner-search-side @if(auth()->check())user-icon @endif">
				...
				<div class="hedaer_bnt">
					<div class="all_doc_btn">
						<!-- <a href="{{ route('register') }}"
							class="cta_dark">{{ $data['button1'] ?? 'Crear documento' }}</a> -->
							<a href="{{ route('register') }}"
							class="cta_dark">{{ 'Create account' }}</a>
					</div>
					@if(auth()->user())
                    @php
                        $unread = auth()->user()->unreadNotifications()->limit(10)->get();
				...
				<div id="dynamic_menu" style="display:none;">
					<div class="right_menu">
						<ul>
							...
							<li>
								{{-- <a href="{{ route('help.center') }}">{{ $data['ayuda_header'] ?? 'Ayuda' }}</a> --}}
							</li>
						</ul>
					</div>
				</div>
